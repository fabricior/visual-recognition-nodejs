casper.start('http://localhost:4701/thermometer', function(result) {
  casper.test.assert(result.status == 400, 'no score gets a 400');
});

casper.thenOpen('http://localhost:4701/thermometer?score=0.5', function(result) {
  casper.test.assert(result.status == 200, 'valid score gets a 200');
  //casper.test.assert(body.score == 0.5, 'score is returned');
});

casper.thenOpen('http://localhost:4701/thermometer?score=-1', function(result) {
